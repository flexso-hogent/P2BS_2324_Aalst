sap.ui.define([
  ...
  return Controller.extend("flexso.controller.Registersession", {
      ...
      jQuery.ajax({
        ...
        success: function (data) {
          if (data.value.length > 0) {
            ...
            MessageToast.show(that.getView().getModel("i18n").getProperty("profileUD"));
          }
        },
        error: function (xhr, status, error) {
          MessageToast.show(that.getView().getModel("i18n").getProperty("fetchuserdate") + error);
        },
      });
    },
    ...
    onRegisterPress: function () {
      ...
      if (this.isUserAlreadyRegistered(oSessionModel.getProperty("/sessionID"))) {
        sap.m.MessageBox.error(this.getView().getModel("i18n").getProperty("registersessionalready"));
      } else {
    ...
    isUserAlreadyRegistered: function (sessionId) {
      ...
      jQuery.ajax({
        ...
        error: function (xhr, status, error) {
          sap.m.MessageBox.error(that.getView().getModel("i18n").getProperty("registrationdatafail") + error);
        },
      });
    ...
    sendRegistrationToBackend: function (oRegistrationData) {
      ...
      jQuery.ajax({
        ...
        success: function () {
          MessageToast.show(that.getView().getModel("i18n").getProperty("registrationsucces"));
          setTimeout(function () {
            that.onBackToHome();
          }, 1500);
        },
        error: function (xhr, status, error) {
          sap.m.MessageBox.error(that.getView().getModel("i18n").getProperty("registrationfailed") + error);
        },
      });
    },
    ...
    onLogoutPress: function () {
      var that = this;
      sap.m.MessageBox.confirm(that.getView().getModel("i18n").getProperty("logout"), {
        title: "Confirm",
